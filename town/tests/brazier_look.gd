# The brazier, from a camera that stays put.
#
#   godot --path . -s res://tests/brazier_look.gd -- <shots dir>
extends SceneTree
...
cam.FieldOfView = 34
local at = Vector3.new(46, 3.6, -46)
cam.CFrame = CFrame.new(at + Vector3.new(-11, 5.0, 14), at)
""")
		t = 0.0
	elif phase == 1 and t > 4.0:
		get_root().get_texture().get_image().save_png(shots.path_join("brazier.png"))
		print("  -> brazier.png")
		quit(0)
	return false
